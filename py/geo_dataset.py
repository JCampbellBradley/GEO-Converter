...
class GEO_Dataset:
	...
	DICT_KEYS = ["relation", "characteristics"]
	...
	def parse_metadata(self, data_dict):
		for key in data_dict:
			short_key = util.reformat_key(key[key.find('_') + 1:])
			value = data_dict[key]
			
			if(short_key in GEO_Dataset.DICT_KEYS):
				for item in value:
					colon_pos = item.find(":")
					
					relation_type = util.reformat_key(item[:colon_pos])
